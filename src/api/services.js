...
export const doSearch = async (keyword) => {
  ...
  const response = await fetch(apiSearch, requestOptions).then((response) => response.text());
...
export const getCauChuyen = async (from = 1, limit = 3) => {
  ...
  const response = await fetch(endpoint, requestOptions).then((response) => response.text());
